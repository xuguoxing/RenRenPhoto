...
@property (nonatomic) NSMutableDictionary *userAlbumsDic;
@end
...
-(id)init
{
    if (self = [super init]) {
        ...
        if (nil != defaults){
            ...
            self.userInfosDic = [NSMutableDictionary dictionary];
            self.userAlbumsDic = [NSMutableDictionary dictionary];
...
{
    UserInfo *userInfo = [self.userInfosDic objectForKey:uid];
    return userInfo;
}

-(void)saveUserInfo:(UserInfo *)userInfo
{
    [self.userInfosDic setObject:userInfo forKey:[userInfo.uid stringValue]];
}
...
{
    [self saveUserInfos:self.friendsArray];
}

-(NSMutableArray *)getUserAlbumsById:(NSString *)uid
{
    return [self.userAlbumsDic objectForKey:uid];
}

-(void)saveUserAlbums:(NSMutableArray *)albums forId:(NSString *)uid
{
    [self.userAlbumsDic setObject:albums forKey:uid];
}
...
{
    NSString *permissionScope = [[Renren permissionArray] componentsJoinedByString:@","];

    return @{
        @"client_id":kRenRenAppKey,
        @"redirect_uri":kRenRenRedirectUri,
        @"response_type":@"token",
        @"display":@"touch",
        @"call_id":[Renren generateCallId],
...
-(void)getUserInfos:(NSArray *)uids WithCompletion:(void (^)(NSArray *))completion failed:(void (^)(NSError *))failed
{
    if (uids.count == 0) {
        return;
    }
    
    NSArray *fields = @[@"uid",@"name",@"sex",@"headurl",@"hometown_location",@"work_history",@"university_history"];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params addEntriesFromDictionary:[self baseParamsDictionary]];
    [params setObject:@"users.getInfo" forKey:@"method"];
    [params setObject:[uids componentsJoinedByString:@","] forKey:@"uids"];
    [params setObject:[fields componentsJoinedByString:@","] forKey:@"fields"];
    [params setObject:[Renren generateSig:params secretKey:_sessionSecret] forKey:@"sig"];
    AFHTTPRequestOperation *request = [[MyHTTPClient sharedClient] HTTPRequestWithMethod:@"POST" path:kRenRenRestServerURL parameters:params];
    [request setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSString *responseString = [[NSString alloc]initWithData:responseObject encoding:NSUTF8StringEncoding];
        NSLog(@"UserInfosData:%@",responseString);
        ...
        if (jsonObject && error == nil) {
            [Renren checkRenrenObject:jsonObject error:&error];
        }
        if (jsonObject && error == nil) {
            if (completion) {
                NSMutableArray *array = [NSMutableArray array];
                for (NSDictionary *userInfo in (NSArray*)jsonObject) {
                    UserInfo *feed = [[UserInfo alloc]initWithDictionary:userInfo];
...
{
    if (self.isSessionValid) {
        return [self _getFriendIdsWithCompletion:completion failed:failed];
    }else {
        [self authorizationWithPermission:[Renren permissionArray] withCompletionBlock:^(BOOL success){
            if (success) {
...
-(void) _getFriendIdsWithCompletion:(void(^)(NSArray *friendsArray))completion failed:(void(^)(NSError *error))failed
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params addEntriesFromDictionary:[self baseParamsDictionary]];
...
-(void)getPhotoFeedsWithPage:(NSInteger)page completion:(void(^)(NSArray *feedsArray))completion failed:(void(^)(NSError *error))failed{
    ...
    [params setObject:@"30" forKey:@"type"];
    ...
    [request setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (jsonObject && error == nil) {
            if (completion) {
                NSMutableArray *array = [NSMutableArray array];
                for (NSDictionary *feedInfo in (NSArray*)jsonObject) {
                    PhotoFeed *feed = [[PhotoFeed alloc]initWithDictionary:feedInfo];
                    [array addObject:feed];
                }
                NSMutableArray *uids = [NSMutableArray array];
                for (PhotoFeed *feed in array) {
                    NSString *uid = [feed.uId stringValue];
                    if ( [uids indexOfObject:uid]==NSNotFound && [[Renren sharedRenren] getUserInfoById:uid]==nil) {
                        [uids addObject:uid];
                    }
                }
                if (uids.count > 0) {
                    [[Renren sharedRenren] getUserInfos:uids WithCompletion:^(NSArray *userInfos) {
                        [[Renren sharedRenren] saveUserInfos:userInfos];
                        completion((NSArray*)array);
